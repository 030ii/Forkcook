...
<div class="fk-heading">현장주문 관리 페이지</div>
<p class="fk-desc">총 ${nowTotalCount}개의 주문이 있습니다</p>
<table class="uk-table uk-table-hover uk-table-divider uk-table-middle uk-table-striped fk-table">
    ...
    <tbody>
    	<c:forEach var="dto" items="${list}" varStatus="status">
   			<c:if test="${dto.snum == adminLoginInfo.snum && dto.ordertype=='A' && dto.orderstate != 4}">
		        <tr>
		        	<td>${nowTotalCount}</td>
					<c:set var="nowTotalCount" value="${nowTotalCount-1}"/>
					<td>${dto.ordernum}</td>
					<td>${dto.uname}</td>
